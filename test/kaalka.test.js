...
describe('Kaalka Encryption Library', () => {
  ...
  const testBinFile = path.join(__dirname, 'test_file.bin');
  const testUtf8File = path.join(__dirname, 'test_utf8.txt');
  const testLargeFile = path.join(__dirname, 'test_large.txt');
  const createdFiles = [];
  ...
  afterAll(async () => {
    ...
    for (const f of [testFile, testBinFile, testUtf8File, testLargeFile]) {
      try { await fs.unlink(f); } catch {}
    }
  });
  ...
  createdFiles.push(encFile);
  expect(encFile.endsWith('.kaalka')).toBe(true);
  const decFile = await k.decrypt(encFile);
  createdFiles.push(decFile);
  const exists = await fs.stat(decFile).then(() => true, () => false);
